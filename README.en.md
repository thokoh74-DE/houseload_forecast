...
> **NEW v2.1.3 – Live-load correction:** The PV/load forecast can be too slow to flag an imminent cutoff during consumption spikes, because it relies on historical hourly averages rather than real instantaneous consumption. As of v2.1.3, the current instantaneous load (`hauslast_aktuell_sensor`, if configured) feeds directly into the simulation for the *current* hour as `max(historical average, live value)`. PV recharging the next morning continues to be correctly accounted for.
...
### v2.1.3
- **Live-load correction** for `sensor.hlf_battery_runtime`: current instantaneous consumption (`hauslast_aktuell_sensor`) now feeds directly into the PV-aware runtime simulation for the current hour — consumption spikes are detected immediately without losing next-morning PV recharging
